...
async function get_info(callback){
  con.query('SELECT * FROM intercambio_estudiantil_salida where validar=1', function(error, results, fields) {
		  if (results != null) {
...
const exportData = async (req, res) => {
  ...
  const worksheet = workbook.addWorksheet("Intercambio Estudiantil de Salida"); // New Worksheet  
  ...
  get_info(function(rows){
    ...
    try {  
      var data = workbook.xlsx.writeFile( path + "/reporte-intercambio-salida.xlsx")
   .  then(() => {
        res.download(path + "/reporte-intercambio-salida.xlsx");
     });
     }catch (err) {
       res.send({
        status: "error",
